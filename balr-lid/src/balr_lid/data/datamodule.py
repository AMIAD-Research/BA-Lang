"""LightningDataModule wiring `LanguageIdDataset` + `BalancedBatchSampler` together.

`train_dataset`, `val_dataset`, `test_dataset` and `train_sampler` are Hydra
sub-configs (they carry a `_target_` and are instantiated here), so the same
code serves both corpora — only `configs/data/{fleurs,tidylang}.yaml` differ
(manifest paths, label list, segment lengths, batch size).
"""
